...
function [Ktrans,vp,ve,tau,chi2] = mTK_bolus_delay_DCEMRI(Cp,Ct,t);
...
ub=[10 0.5 1 0.5];
Ktrans = 0.01; vp = 0.01; ve = 0.4; tau = 0.11;
x0=[Ktrans vp ve tau];
...
% ------------------------------------------------------------------------------------
[x,resnorm,residue,flag] = lsqnonlin(@fit_mTK_bolus_delay_model, x0, lb, ub, options, t, Ct, Cp);
Ktrans = x(1);
vp = x(2);
ve = x(3);
tau = x(4);
...
if margin > 0
    ...
    x0=[Ktrans vp ve 0.11];         
    [x1,resnorm1] = lsqnonlin(@fit_mTK_bolus_delay_model, x0, lb, ub, options, t, Ct, Cp);

    x0=[Ktrans vp ve 0.12];      
    [x2,resnorm2] = lsqnonlin(@fit_mTK_bolus_delay_model, x0, lb, ub, options, t, Ct, Cp);

    x0=[Ktrans vp ve 0.12];      
    [x3,resnorm3] = lsqnonlin(@fit_mTK_bolus_delay_model, x0, lb, ub, options, t, Ct, Cp);
    
    x0=[Ktrans vp ve 0.13];      
    [x4,resnorm4] = lsqnonlin(@fit_mTK_bolus_delay_model, x0, lb, ub, options, t, Ct, Cp);
...
margin = sum(abs(residue-Ct)./Ct)/length(Ct);
if margin > 0
    % Try different initial conditions for Ktrans, Vp, Ve, tau.
    x0=[0.01 0.2 0.2 tau];         
    [x1,resnorm1] = lsqnonlin(@fit_mTK_bolus_delay_model, x0, lb, ub, options, t, Ct, Cp);
...
%figure; plot(t,Ct,'k.'); hold on; plot(t,Ct_mTK_bolus_delay_model (t, Cp, [Ktrans vp ve tau]));
